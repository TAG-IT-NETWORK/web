...
The NTAG 424 DNA is TAG IT's primary chip for CLASS 2 and CLASS 3 applications, providing hardware-backed cryptographic authentication at commercial price points.
...
SUN (Secure Unique NFC) provides cryptographic proof that a specific chip was scanned:

```
URL Format:
https://verify.tagit.network/scan?
...
|-----------|-------------|
| `uid` | Unique chip identifier (7 bytes) |
| `cnt` | Scan counter (increments each read) |
| `cmac` | AES-128 CMAC signature |

The CMAC is computed over: `uid || cnt || random` using the chip's secret key — proving the physical tag was present during scanning.
...
sequenceDiagram
    ...
    Note over Tag,Phone: Step 1: Physical Scan
    Phone->>Tag: Read UID + SUN message
    Tag-->>Phone: uid, cnt, cmac
    
    Note over Phone,API: Step 2: Verify Signature
    Phone->>API: POST /verify {uid, cnt, cmac}
    API->>API: Validate CMAC with stored key
    API-->>Phone: {valid: true, tokenId: 12345}
    
    Note over API,Chain: Step 3: On-chain Lookup
    API->>Chain: TAGITCore.verify(tokenId, signals)
    Chain-->>API: VerifyResult{passed: true}
    
    Note over Phone: Step 4: Display Result
    API-->>Phone: Full verification response
    Phone->>Phone: Show ✅ VERIFIED
```
...
Order NTAG 424 DNA tags from an authorized converter (GoToTags for pilot, Smartrac for volume).

### 2. Provision Keys

Use TAG IT's key injection service or self-provision with HSM:

```javascript
// Example: Generate and inject AES-128 key
const key = crypto.randomBytes(16);
await tag.authenticate(FACTORY_KEY);
await tag.changeKey(KEY_SLOT_1, key);
await tagitAPI.registerKey(tag.uid, key);
```

### 3. Configure NDEF

Set the SUN URL template:

```
https://verify.tagit.network/s?u={uid}&c={cnt}&m={cmac}
```

### 4. Bind to Blockchain

```javascript
// After physical tag is programmed
const tokenId = await tagitCore.mint(metadata);
const tagHash = keccak256(uid, key);
await tagitCore.bindTag(tokenId, tagHash, signature);
```
...
- [Getting Started](/docs/getting-started/quickstart) — First integration
- [SDK Reference](/docs/sdk/javascript) — JavaScript SDK
- [API Reference](/docs/api/verification) — Verification endpoints

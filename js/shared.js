...
// Development mode check - enables console logging on localhost only
...
// Intersection Observer & Animations
    ...
    // FIXED: Use classList instead of inline transform to preserve centering
    legendItems.forEach(item => {
      ...
      item.addEventListener('mouseenter', () => {
        if (ringNum) {
          ...
          if (ring) {
            ring.classList.add('highlighted');
          }
        }
      ...
      item.addEventListener('mouseleave', () => {
        if (ringNum) {
          const ring = document.querySelector(`.ring-${ringNum}`);
          if (ring) {
            ring.classList.remove('highlighted');
          }
        }
    ...
    // ═══════════════════════════════════════════════════════════════
    // JOURNEY SECTION - Two Column Navigation
    // ═══════════════════════════════════════════════════════════════
    const journeySection = document.getElementById('use-cases');
    if (journeySection) {
      const industryItems = journeySection.querySelectorAll('.industry-item');
      const stageItems = journeySection.querySelectorAll('.stage-item');
      const chapterPanels = journeySection.querySelectorAll('.chapter-panel');
      const prevBtn = journeySection.querySelector('.btn-prev');
      const nextBtn = journeySection.querySelector('.btn-next');

      let currentIndustry = 'luxury';
      let currentStage = 1;
      const totalStages = 5;

      function updateJourneyUI() {
        // Update industry items
        industryItems.forEach(item => {
          item.classList.toggle('active', item.dataset.industry === currentIndustry);
        });

        // Update stage items
        stageItems.forEach(item => {
          const stage = parseInt(item.dataset.stage);
          item.classList.toggle('active', stage === currentStage);
        });

        // Update chapter panels - show only matching industry + stage
        chapterPanels.forEach(panel => {
          const isMatch = panel.dataset.industry === currentIndustry &&
                          parseInt(panel.dataset.stage) === currentStage;
      ...
      function setIndustry(industry) {
        ...
        currentStage = 1; // Reset to stage 1 when switching industry
        updateJourneyUI();
      }

      // Industry item clicks
      industryItems.forEach(item => {
        item.addEventListener('click', () => {
          setIndustry(item.dataset.industry);
        });
      });
      ...
      stageItems.forEach(item => {
        item.addEventListener('click', () => {
          goToStage(parseInt(item.dataset.stage));
        });
      });
...
// ═══════════════════════════════════════════════════════════════
      ...
      if (savedType === 'metamask' && typeof window.ethereum !== 'undefined') {
        try {
          ...
          if (accounts.length > 0 && accounts[0].toLowerCase() === savedAddress.toLowerCase()) {
            ...
            updateNavButton();
            if (isDev) console.log('Wallet session restored');
          } else {
            clearWalletState();
          }
        } catch (e) {
          if (isDev) console.log('Could not restore wallet session:', e);
          clearWalletState();
        }
      }
    }
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      // Initialize RWA section animations
      initRWAAnimations();
    });

    // ═══════════════════════════════════════════════════════════════
    // RWA SECTION ANIMATIONS
    // ═══════════════════════════════════════════════════════════════

    function initRWAAnimations() {
      const rwaSection = document.querySelector('.rwa-section');
      if (!rwaSection) return;

      // Initialize animated counter
      initAnimatedCounter();

      // Initialize market cards with 3D tilt and scroll triggers
      initMarketCards();

      // Initialize value list animations
      initValueListAnimations();

      // Generate floating particles
      generateRWAParticles();
    }

    // Animated Counter for $16T stat
    function initAnimatedCounter() {
      const statNumber = document.querySelector('.rwa-stat-hero .stat-number');
      if (!statNumber) return;

      const targetValue = parseInt(statNumber.dataset.target) || 16;
    ...
    function animateCounter(element, target) {
      ...
      function updateCounter(currentTime) {
        ...
        // Update only the number part, keep the structure
        if (currencySpan && suffixSpan) {
          element.innerHTML = `<span class="currency">$</span>${currentValue}<span class="suffix">T+</span>`;
        } else {
          element.textContent = `$${currentValue}T+`;
        }
    ...
    // Market Cards with 3D Tilt and Scroll Animations
    function initMarketCards() {
      const marketCards = document.querySelectorAll('.rwa-section .market-card');
      ...
      const cardObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
          if (entry.isIntersecting) {
            ...
            const barFill = entry.target.querySelector('.market-bar-fill');
            if (barFill) {
              const width = barFill.style.width || barFill.dataset.width || '70%';
    ...
    // Value List Staggered Animations
    function initValueListAnimations() {
      const valueItems = document.querySelectorAll('.rwa-section .value-item');
      if (valueItems.length === 0) return;

      const listObserver = new IntersectionObserver((entries) => {
        entries.forEach(entry => {
          if (entry.isIntersecting) {
            // Get delay from data attribute or calculate based on index
    ...
    // Generate Floating Particles for RWA Section
    function generateRWAParticles() {
      const particleField = document.querySelector('.rwa-section .particle-field');
      ...
      for (let i = 0; i < particleCount; i++) {
        ...
        particle.style.background = Math.random() > 0.5 ?
          'var(--electric-blue)' : 'var(--circuit-green)';
